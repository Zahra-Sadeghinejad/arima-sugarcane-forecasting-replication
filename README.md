...
Across lags 1–20, **7 reproduced ACF values differ by no more than 0.003**. Another **5 differ by more than 0.003 but no more than 0.02**, meaning **12 of 20 are within 0.02 overall**. The remaining **8 lags differ by more than 0.02**.
...
The full lag-by-lag comparison is generated by `save_outputs.R` and stored in [`results/acf_comparison.csv`](results/acf_comparison.csv).
...
To regenerate all committed output files — including the seven result tables and nine figures — run from the repository root:
...
```text
Reproducible outputs generated: 7 result tables in results/ and 9 figures in figures/.
```
